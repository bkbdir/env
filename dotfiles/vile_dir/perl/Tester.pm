...
@ISA      = 'Vile::Exporter';
%REGISTRY = (
    tester => [ \&tester, 'flat hierarchical directory listing' ],
    'tester-help' => [ sub { &manual }, 'manual page for tester' ]
);
...
sub tester {
    ...
    while ( !defined($root) ) {
        $root =
          Vile::mlreply_dir( "Directory to search in? ", $tester_oldroot );
    ...
    my $resbuf = $curbuf->edit("tester $root $fpat");
...
tester - list a directory hierarchy
...
list populate a buffer named I<tester-.-*>.
