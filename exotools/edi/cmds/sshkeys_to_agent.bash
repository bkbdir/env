...
for key in $HOME/.ssh/*.pub; do
   bkey=$(basename $key)
   pkey=${bkey%.*}
   [ -f "$HOME/.ssh/$pkey" ] || { echo "Err: priv. key $pkey not exist"; exit 1; }
   pw=$(perl $script "$pkey" 'ben.A_special25v151001.1' "$master" )
   perl $script "$pkey" 'ben.A_special25v151001.1' "$master" | xsel -b 
   echo "Paste from clipboard for key $pkey"
   eval `keychain --eval $pkey`
